...
class DeleteUserAccount extends ConsumerStatefulWidget {
  ...
  const DeleteUserAccount({required this.userProfile, Key? key}) : super(key: key);
...
class DeleteUserAccountWidget extends ConsumerState<DeleteUserAccount> {
  ...
  final _callErrors = StateProvider<List<CallError>>((ref) => []); //TODO: implementation of error handling is open!
  ...
  Widget build(BuildContext context) {
    ...
    fields.add(Padding(
      ...
      child: Text("DeleteUserAccountWarning".tr(), textAlign: TextAlign.center, style: textStyleTitleMedium.apply(color: Colors.red)), //TODO: replace with code label
    ));
    ...
    fields.add(Padding(
      ...
      child: CheckboxListTile(
        ...
        title: Text("IConfirm".tr(), textAlign: TextAlign.center, style: textStyleTitleLarge.apply(color: Colors.red)),
    ...
    //Error Message
    if (isError) {
      fields.add(ApiErrorMessages(callErrors: ref.watch(_callErrors), deltaFontSize: ref.watch(deltaFontSizeProvider)));
    }

    //Delete User Account Button
    fields.add(Consumer(builder: (BuildContext context, WidgetRef ref, Widget? child) {
      //child:
      return Padding(
        ...
        child: formState == AppFormState.processing
            ...
            : SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: ref.watch(_isDeleteConfirmedProvider) ? () => _deleteUserAccount(context, ref) : null,
                  child: Padding(
                      padding: const EdgeInsets.all(10.0),
                      child: FittedBox(
                          fit: BoxFit.scaleDown,
                          child: Text("Delete".tr(),
                              textAlign: TextAlign.center, style: textStyleHeadlineSmall.copyWith(color: ref.watch(_isDeleteConfirmedProvider) ? Colors.white : null)))),
                )),
      );
    }));
  ...
  _deleteUserAccount(BuildContext context, WidgetRef ref) {
    ...
    ref.read(deleteUserAccountRequestProvider.notifier).state = DeleteUserAccountRequest(userName: userProfile.userName);
    ...
    deleteResult.then((data) {
      if (data.resultCode == AppResultCode.ok) {
        ...
        Navigator.pop(scaffoldKeyDeleteUserAccount.currentContext!);
        Navigator.pop(scaffoldKeyDeleteUserAccount.currentContext!);
      } else {
