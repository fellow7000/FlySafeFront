...
class MainScreenWgt extends ConsumerState<MainScreen> {
  ...
  void initState() {
    ...
    if (!kIsWeb && !kDebugMode && ref.read(isShowStartDialogProvider.notifier).state) {
      Future.delayed(Duration.zero, () => StartDialog().startDialog(
          context: context,
          ref: ref,
          title: "${"Warning".tr()}!",
          dialogText: "${"AppAboutTxt".tr()}\n\n${"ContinueConfirmation".tr()}",
  ...
  Widget build(BuildContext context) {
    ...
    //authorize if needed
    if (ref.watch(isStartConfirmedProvider) && ref.watch(appModeProvider) == AppMode.online && (ref.watch(authStateProvider) == LogAs.user || ref.watch(authStateProvider) == LogAs.club) && requestStartUpSignIn) {
        ...
        mainScreen = ref.watch(signInProvider).when(
            ...
            error: (err, stack) =>
                MainPanel(widgetList: [ApiErrorTileRetry(
                  err: err,
            ...
            loading: () => MainPanel(widgetList: [AppProcessIndicator(message: "SigningIn".tr())]));
    } else if (ref.watch(isStartConfirmedProvider)) {
      mainScreen = MainPanel(widgetList: widgetList);
    }

    return WillPopScope(
      onWillPop: () => ExitDialog.exitConfirmation(
          context: context,
          ref: ref,
          title: "Confirm".tr(),
